...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const userInitial = fullName ? fullName[0].toUpperCase() : (user.email ?? "U")[0].toUpperCase();
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <div className="relative" ref={menuRef}>
              <button
                ...
                title={user.email}
                className="flex items-center justify-center h-8 w-8 rounded-xl text-xs font-bold text-white transition-all duration-200 hover:scale-105"
                style={{
                  background: "linear-gradient(135deg, #e9434a 0%, #c93040 100%)",
                  boxShadow: "0 2px 8px rgba(233,67,74,0.35)",
                }}
              >
                {userInitial}
              </button>
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-10 w-52 rounded-xl border border-gray-100 bg-white shadow-xl z-50 overflow-hidden">
                  {/* User info */}
                  <div className="px-4 py-3 border-b border-gray-50 bg-gray-50/60">
                    <div className="flex items-center gap-2.5">
                      <div
                        ...
                        style={{ background: "linear-gradient(135deg, #e9434a 0%, #c93040 100%)" }}
                      >
                        {userInitial}
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs font-semibold text-gray-800 truncate">{displayName}</p>
                        <p className="text-[10px] text-gray-400 truncate">{fullName ? user.email : "Signed in"}</p>
                      </div>
                    </div>
                  </div>
      ...
      <ChatPanel open={chatOpen} onClose={() => setChatOpen(false)} pageContext={pageContext} userId={user.id} />
    </div>
  );
}
